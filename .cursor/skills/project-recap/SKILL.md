...
**Optional hero image** — generate a hero banner using a Gemini 3+ subagent (16:9 recommended) that visually captures the project's identity or domain. Match the style to the page's palette. Embed as base64 data URI using the `.hero-img-wrap` pattern from css-patterns.md. Place above or just below the title. Skip if image generation isn't available — the page should stand on its own.
