...
**Optional illustrations** — consider generating a conceptual illustration of the planned system using a Gemini 3+ subagent when it would help the reader visualize the change. Embed as base64 data URI. See css-patterns.md "Generated Images" for container styles. Skip if image generation isn't available or the plan is purely structural.
